...
def pipeline(tokenizer, model):
    ...
    train_loader, dev_loader = get_dataloaders(tokenizer=tokenizer, batch_size=k_batch_size, num_train=k_num_train,
                                               num_val=k_num_val, data_dir=k_data_dir, num_workers=k_num_workers)
    ...
    while epoch < k_epochs:
        ...
        with torch.no_grad(), tqdm(total=num_val) as progress_bar:
            for batch_num, batch in enumerate(dev_loader):
                ...
                # Collect some stats
                total_matches_no_eos, total_matches_with_eos, correct_indices = util.masked_token_match(tgt_ids,
                                                                                                        generated_ids,
                                                                                                        return_indices=True)
...
torch.save(model, ".UnifiedQAfinetune.pt")